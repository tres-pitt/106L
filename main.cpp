...
void do_update1(SimpleGraph &sg) {
    double ka = 0.001;
    double kr = 0.001;
    Node n0, n1;
    Edge e;
    for (unsigned int i=0; i<sg.edges.size(); i++) {
        ...
        n0 = sg.nodes[e.start];
        n1 = sg.nodes[e.end];
        double frep = f_rep(n0, n1, kr);
        double theta = calc_theta(n0, n1);
        double xval = frep*cos(theta);
        double yval = frep*sin(theta);

        double fatt = f_att(n0, n1, ka);
        double xval2 = fatt * cos(theta);
        double yval2 = fatt * sin(theta);

//        sg.nodes[e.start].x -= xval;
//        sg.nodes[e.start].y -= yval;
//        sg.nodes[e.end].x += xval;
//        sg.nodes[e.end].y += yval;


        sg.nodes[e.start].x  += xval2;
        sg.nodes[e.start].y += yval2;
        sg.nodes[e.end].x -= xval2;
        sg.nodes[e.end].y -= yval2;

//        sg.nodes[e.start].x += (xval2 - xval);
//        sg.nodes[e.start].y += (yval2 - yval);
//        sg.nodes[e.end].x += (xval - xval2);
//        sg.nodes[e.end].y += (yval - yval2);


//        sg.nodes[e.start].x  = sg.nodes[e.start].x - xval + xval2;
//        sg.nodes[e.start].y = sg.nodes[e.start].y - yval + yval2;
//        sg.nodes[e.end].x = sg.nodes[e.end].x + xval - xval2;
//        sg.nodes[e.end].y = sg.nodes[e.end].y + yval - yval2;
    }
}

//repulse - for every pair of nodes
void do_update2(SimpleGraph &sg, const vector<struct Node> &nvec) {
    double kr = 0.001;
    Node n0, n1;
    Edge e;
    double f, theta;
...
void update_graph(vector<struct Force> &forces, SimpleGraph &sg) {
    ...
    for (unsigned int i=0; i<nodez.size()-1; i++) {
//        Node n = nodez[i];
        Force f = forces[i];
        sg.nodes[i].x += f.dx;
        sg.nodes[i].y += f.dy;
